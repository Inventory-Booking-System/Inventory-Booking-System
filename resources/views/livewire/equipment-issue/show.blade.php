        ...
        <div class="row">
            <div class="card w-100 mr-3">
                <div class="card-header bg-dark text-center">
                    <h1>{{ $equipmentIssue->title }}</h1>
                </div>
                <div class="card-body">
                    <strong>Created Date:</strong><p class="card-text">{{ $equipmentIssue->humanFormat($equipmentIssue->created_at) }}</p>
                    <strong>Last Updated:</strong><p class="card-text">{{ $equipmentIssue->humanFormat($equipmentIssue->updated_at) }}</p>
                </div>
            </div>
        </div>
    </div>
        ...
        <x-table>
            <x-slot name="head">
                <x-table.row>
                    ...
                    <x-table.heading sortable wire:click="sortBy('distribution_groups.name')" :direction="$sorts['distribution_groups.name'] ?? null" class="col-2">User</x-table.heading>
                    ...
                    <x-table.row wire:key="row-{{ $incident->id }}">
                        ...
                        <x-table.cell class="col-2">
                            @foreach($incident->issues as $issue)
                                <x-link route="equipmentIssues" id="{{ $issue->id }}" value="x{{ $issue->pivot->quantity }} {{ $issue->title }} (£{{ $issue->cost }})"></x-link><br>
                            @endforeach
                        </x-table.cell>
                    </x-table.row>
                @empty
